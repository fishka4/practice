...
public class GoogleSheets {
    ...
    public static Object[][] getData(String sheetID, String range) throws IOException {
        Sheets service = getSheetsService();
        ValueRange response = service.spreadsheets().values().get(sheetID, range)
                .execute();

        List<List<Object>> values = response.getValues();
        Object[][] valuesObject = new Object[values.size()][];
        for(int i = 0; i < values.size(); i++){
            valuesObject[i]= values.get(i).toArray();
        }
...
//        String range = "Class Data!A2:E";
        ...
        Object[][] values = getData(spreadsheetId, range);
